...

{
 ...
 // Max number of entries to return.
    ...
    function getDBColumns()
    {
        ...
        $rs = db2_columns($GLOBALS['conn'], null, $_SESSION['Db2Schema'], strtoupper($this->tableName), '%');
    ...
    {
        Trace::traceComment(null, __METHOD__);
        $rs = db2_special_columns($GLOBALS['conn'], null, $_SESSION['Db2Schema'], $this->tableName, 0);
        while ($row = db2_fetch_assoc($rs)) {
    ...
    {
        Trace::traceComment(null, __METHOD__);
        $rs = db2_primary_keys($GLOBALS['conn'], null, $_SESSION['Db2Schema'], $this->tableName);
        while ($row = db2_fetch_assoc($rs)) {
            // print_r($row);
    ...
    {
        Trace::traceComment(null, __METHOD__, __LINE__);
        $sql = " SELECT count(*) as RECORDS FROM " . $_SESSION['Db2Schema'] . ".$this->tableName WHERE " . $this->buildKeyPredicate($record);
        Trace::traceVariable($sql, __METHOD__, __LINE__);
        $rs = $this->execute($sql);
        if (! $rs) {
            return false;
