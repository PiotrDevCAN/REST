...
var configDefaults = {
    ...
    faces: {
        ...
        onclick: function(person) {
            location.href='http://faces.tap.ibm.com/#uid:'+person.uid+'/'+person.name;
...
var TypeAhead = function (inputBox, settings) {
    ...
    if ( settings.features.faces && settings.faces.prefixDom ) {
        this.container.append(settings.faces.prefixDom);
    }
    ...
    for ( var f in settings.features ) {
        this.resultLists[f] = 
            h.queryAll(document.createElement('ul')).addClass(f+'-typeahead-results').appendTo(this.container);
    }
...
//Can't assume faces anymore
//        resultsJson['persons'] = resultsJson['persons'] || [];
    ...
    function getSequential(currItem, prop) {
        if ( currItem ) {
            ...
            var children, list = currItem.parentNode;
            while ( (list = list[prop]) && list.nodeName.toUpperCase() == 'UL' ) {
    ...
    var highlightNext = function() {
        ...
        if ( this.isPopupShowing() ) {
            ...
            //Highlight first item
                ...
                _highlight(null, h.query(h.query(this.container, 'ul:nth-of-type(1)'), 'li:first-child'));
            }
        }
    }.bind(this);
    ...
    var renderResults = function(feature, resultsJson, query) {
        ...
        if ( settings.features.faces && feature == 'faces' ) {
            ...
            if (results.length) {
                ...
                if(c.showMoreResults){
                    var moreResults = getMoreResultsMessage(resultsJson, results.length, query);
                    if ( moreResults ) {
        ...
        var topQueries = [];
        var w3icon = "http://w3.ibm.com/jct03001pt/w3ODWThemeSkin/themes/html/w3ODWTheme/images/favicon.ico";
        ...
        if ( settings.features.topsearch && feature == 'topsearch' ) {
            ...
            if (numResults > 0) {
                ...
                for (var i = 0; i < Math.min(resultsJson.length, settings.topsearch.maxResults); i++) {
                    ...
                    link.innerHTML = '<img src="' + w3icon + '" class="icon" /><span class="query">' + currentQuery.title + '</span> ' + '<span class="url">' + currentQuery.url + '</span>';
                    li.appendChild(link);
