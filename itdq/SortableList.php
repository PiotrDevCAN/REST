...
//         return $bgColor;
    ...
    function displayProfileOptions()
    {
        if ($this->profileSaveable && isset(AllITdqTables::$PROFILES)) { // They are setup to have saved profiles.
            $loader = new Loader();
            $myProfiles = $loader->load('PROFILE_NAME', AllITdqTables::$PROFILES, " (INTRANET='" . db2_escape_string(trim($GLOBALS['ltcuser']['mail'])) . "' or INTRANET='global' ) AND PAGE='" . db2_escape_string(trim($_SERVER['PHP_SELF'])) . "' ");
            ...
            if (isset($_SESSION['adminBg'])) {
                if (employee_in_group($_SESSION['adminBg'], $GLOBALS['ltcuser']['mail'])) {
            ...
            if (isset($_REQUEST['sbsLoad_Profile'])) {
                if (! empty($_REQUEST['sbsLoad_Profile'])) {
                    $allFields = $loader->loadIndexed('FIELD_VALUE', 'FIELD_NAME', AllITdqTables::$PROFILES, " (INTRANET='" . db2_escape_string(trim($GLOBALS['ltcuser']['mail'])) . "' OR INTRANET='global')  AND PAGE='" . db2_escape_string(trim($_SERVER['PHP_SELF'])) . "'  AND PROFILE_NAME='" . db2_escape_string(trim($_REQUEST['sbsLoad_Profile'])) . "' ");
